...
export function insert({ modelName, videoPath, audioPath, voiceId, voicePresetId }) {
  const db = connect()
  const stmt = db.prepare(
    'INSERT INTO f2f_model (name, video_path, audio_path, voice_id, voice_preset_id, created_at) VALUES (?, ?, ?, ?, ?, ?)'
  )
  const info = stmt.run(modelName, videoPath, audioPath, voiceId, voicePresetId, Date.now())
